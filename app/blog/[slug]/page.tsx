...
type PostContentComponent = (props: { html: string }) => ReactNode;

/** Articles whose HTML is rendered through figure-aware components. */
const articleContent: Record<string, () => Promise<{ default: PostContentComponent }>> = {
  '2026-09-19': () => import('@/components/jev-bench/PostContent'),
};
...
export default async function BlogPost({ params }: PageProps) {
  const { slug } = await params;
  const post = await getPost(slug);
  const loadArticleContent = articleContent[slug];
  const PostContent = loadArticleContent
    ? (await loadArticleContent()).default
    : hasOrchestraPlaceholders(post.contentHtml)
  ...
  return (
    ...
    >
      <Container maxWidth="md" component="section" sx={{ pt: 4, pb: 6 }}>
        ...
        <Box
          component="article"
          data-figure-article={loadArticleContent ? '' : undefined}
          sx={{
            mt: 2,
            '& h1, & h2, & h3, & h4, & h5, & h6': {
              color: 'text.primary',
