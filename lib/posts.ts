...
import { inlineArticleFigures } from './inline-figures';
...
export async function getPost(slug: string): Promise<Post> {
  ...
  const contentHtml = sanitizeHtml(rawContentHtml, {
    // KaTeX uses inline SVG with case-sensitive attributes for radicals.
  ...
  return {
    slug,
    contentHtml: inlineArticleFigures(contentHtml, slug),
    ...metadata,
  };
}
